...
export function registerCoordinationTools(server: McpServer, env: Env): void {
  ...
  server.tool(
    ...
    "Add a task with priority and optional assignee.",
    {
      ...
      shared:      z.boolean().optional().default(false).describe("If true, visible to partner via bridge."),
    },
    async (input) => {
      const id = generateId();
  ...
  server.tool(
    ...
    async (input) => {
      ...
      return { content: [{ type: "text", text: JSON.stringify({ ok: true, id: input.id, status: input.status }) }] };
    },
  );
  ...
  server.tool(
    ...
    async (input) => {
      ...
      await env.DB.prepare(`
        INSERT INTO lists (id, list_name, item_text, added_by, added_at, completed, shared)
        VALUES (?, ?, ?, ?, ?, 0, ?)
      `).bind(id, input.list_name, input.item_text, input.added_by ?? null, now, input.shared ? 1 : 0).run();
  ...
  server.tool(
    "halseth_list_item_complete",
    "Mark a list item as completed.",
    {
      id: z.string().describe("The list item ID."),
    },
    async (input) => {
      const now = new Date().toISOString();
      const result = await env.DB.prepare(
      ...
      if (result.meta.changes === 0) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "List item not found" }) }] };
      }
      return { content: [{ type: "text", text: JSON.stringify({ ok: true, id: input.id }) }] };
    },
  );
