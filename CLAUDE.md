...
| -- | `0114` | `handover_packets.close_kind` (NULL = authored live; `reconstructed`/`empty`/`machine_opened` = backfilled) + `sessions.opened_by`. **All five global latest-handover reads filter `close_kind IS NULL`** — a reconstruction must never be served as the last thing that happened; the per-session read is deliberately unfiltered. `opened_by` closes the provenance gap 0113 left: `surface` says WHERE a session opened, nothing said WHAT opened it, which is why 187 stale rows were unattributable. Same migration's companion change: **six state-READ phrases moved off `session_open`** ("current state", "how am i", "what's my state", "where am i", "show my state", "check my state") onto `triad_state_read` — a read-shaped phrase on a route that INSERTs a session row is how crons and mid-turn agent calls opened lifecycle rows nobody would close. Full write-up: `docs/close-the-187-2026-08-04.md`. |
