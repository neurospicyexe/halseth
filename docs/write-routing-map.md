...
| `tray_draft_read` | execTrayDraftRead | tray.ts | READ | "read draft <id>": ONE owner row in full (any review_state, archived included and labelled) plus provenance -- table, source, text prefix, channel, external_id / thread_key, created/reviewed/edited. Ambiguous prefix (keep/drop too) is refused with the candidates, never guessed |
...
| `conversation_capture` | execConversationCapture | webmind.ts | wm_continuity_notes (note_type conversation_capture, embedded on write) | 2026-08-15 D3: the Claude.ai capture verb; thread_key `capture:<session_id>`; bypasses the 10-min write gate by design; content from context.content ONLY |
